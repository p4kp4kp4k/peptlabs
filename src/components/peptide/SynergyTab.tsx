...
interface Interaction { nome: string; status: string; descricao: string; }
interface InteractionsData { peptideos?: Interaction[]; outras_substancias?: Interaction[]; }
...
function StatusBadge({ status }: { status: string }) {
  ...
  if (upper.includes("SINÉR") || upper.includes("SINERG") || upper.includes("COMPATÍV")) {
    return <Badge className="text-[10px] bg-emerald-500/15 text-emerald-400 border border-emerald-500/25 font-bold px-2">{status}</Badge>;
  }
  if (upper.includes("MONITOR")) {
    return <Badge className="text-[10px] bg-amber-500/15 text-amber-400 border border-amber-500/25 font-bold px-2">{status}</Badge>;
  }
  if (upper.includes("EVITAR")) {
    return <Badge className="text-[10px] bg-red-500/15 text-red-400 border border-red-500/25 font-bold px-2">{status}</Badge>;
  }
              ...
              <tbody>
                {allInteractions.map((item, i) => (
                  <tr key={i} className="border-b border-border/15 last:border-0 align-top hover:bg-secondary/30 transition-colors">
                    <td className="py-3 px-2 text-foreground font-semibold whitespace-nowrap">{item.nome}</td>
                    <td className="py-3 px-2"><StatusBadge status={item.status} /></td>
                    <td className="py-3 px-2 text-muted-foreground leading-relaxed">{item.descricao}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            {stacksData.map((stack, i) => (
              <div key={i} className="p-4 rounded-lg bg-secondary/40 border border-border/15">
                <div className="flex items-center justify-between mb-2.5">
                  ...
                  <Badge className="text-[10px] bg-primary/15 text-primary border border-primary/25 font-semibold">{stack.objetivo}</Badge>
                </div>
